...
export type AuthProviderType = 'google' | 'email' | 'phone' | 'password' | 'passkey' | 'apple';
...
export async function resolveOrCreateUser(params: {
  ...
  // 3. Create brand new canonical user
  ...
  const roleVal = params.role || 'client';
  const emailVerifiedVal = params.verified && provider === 'google' || provider === 'email';
  ...
  const { rows: createdRows } = await sql`
    ...
    RETURNING id, phone, email, first_name, last_name, role, phone_verified, email_verified
  `;
