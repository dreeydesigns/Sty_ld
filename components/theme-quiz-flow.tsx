...
export function ThemeQuizFlow() {
  ...
  if (phase === "intro") {
    return (
      <main className="min-h-screen bg-[var(--surface-main)] px-4 py-8 text-[var(--text-secondary)]">
        <section className="mx-auto grid min-h-[calc(100vh-4rem)] max-w-lg content-center">
          <div className="overflow-hidden rounded-[38px] border border-[var(--border-subtle)] bg-[var(--surface-card)] shadow-[0_24px_80px_rgba(0,0,0,0.2)]">
            {/* Header */}
            ...
            <div className="space-y-3 p-6">
              {[
                ...
                <div
                  key={row.title}
                  className="flex items-start gap-4 rounded-[22px] bg-[var(--surface-elevated)] border border-[var(--border-subtle)] px-4 py-4"
                >
                  <span className="mt-0.5 text-base font-bold text-[var(--color-clay-text)]">{row.icon}</span>
                  <div>
                    <p className="text-sm font-semibold text-[var(--text-primary)]">{row.title}</p>
                    <p className="mt-1 text-xs leading-5 text-[var(--text-secondary)]">{row.copy}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="space-y-3 px-6 pb-7">
              <button
                className="inline-flex min-h-[52px] w-full items-center justify-center gap-2 rounded-full bg-[var(--color-action-primary)] px-6 text-sm font-semibold text-[var(--color-action-primary-text)] transition hover:opacity-90"
                onClick={startQuiz}
                type="button"
              >
              ...
              <Link
                href={safeReturnTo}
                className="inline-flex min-h-[44px] w-full items-center justify-center rounded-full border border-[var(--border-subtle)] px-6 text-sm font-semibold text-[var(--text-secondary)] transition hover:border-[var(--border-strong)] hover:text-[var(--text-primary)]"
              >
                Skip for now
              </Link>
            </div>
        ...
        <section className="mx-auto grid min-h-[calc(100vh-4rem)] max-w-lg content-center">
          <div className="overflow-hidden rounded-[38px] border border-[var(--border-subtle)] bg-[var(--surface-card)] shadow-[0_24px_80px_rgba(0,0,0,0.2)]">
            ...
            <div className="p-6">
              ...
              <p className="mt-4 rounded-[18px] bg-[var(--surface-elevated)] border border-[var(--border-subtle)] px-4 py-3 text-xs leading-6 text-[var(--text-secondary)]">
            ...
            <div className="space-y-3 px-6 pb-7">
              ...
              <button
                className="inline-flex min-h-[44px] w-full items-center justify-center gap-2 rounded-full border border-[var(--border-subtle)] px-6 text-sm font-semibold text-[var(--text-secondary)] transition hover:border-[var(--border-strong)] hover:text-[var(--text-primary)]"
                onClick={() => {
                  setAnswers([]);
                  setStep(0);
                  setResult(null);
  ...
  // ── QUIZ QUESTIONS ─────────────────────────────────────────────────────────
  return (
    <main className="min-h-screen bg-[var(--surface-main)] px-4 py-5 text-[var(--text-secondary)]">
      <section className="mx-auto grid min-h-[calc(100vh-2.5rem)] max-w-3xl content-center">
        <div className="overflow-hidden rounded-[38px] border border-[var(--border-subtle)] bg-[var(--surface-card)] shadow-[0_24px_80px_rgba(0,0,0,0.2)]">
          {/* Question header */}
          <div className="bg-[var(--color-ink)] p-5 text-white">
                ...
                >
                  ...
                  <p
                    className={cn(
                      ...
                      active ? "text-white/85" : "text-[var(--text-secondary)]",
                    )}
                  >
                    {option.copy}
                  </p>
