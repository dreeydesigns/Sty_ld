...
const STATS = [
  { label: "Active listings", value: "12", icon: <Package className="h-5 w-5" />, color: "var(--ms-purple)" },
  { label: "New orders", value: "3", icon: <ShoppingBag className="h-5 w-5" />, color: "var(--ms-rose)" },
  { label: "Earnings this month", value: "KES 24,150", icon: <WalletCards className="h-5 w-5" />, color: "var(--ms-gold)" },
  { label: "Shop rating", value: "4.8 ✦", icon: <Star className="h-5 w-5" />, color: "var(--ms-teal)" },
];
...
const ORDER_STATUS_COLOURS: Record<string, string> = {
  Pending: "var(--ms-orange)",
  "In transit": "var(--ms-gold)",
  Delivered: "var(--ms-teal)",
  Disputed: "var(--ms-rose)",
};

const PRODUCT_STATUS_COLOURS: Record<string, string> = {
  Published: "var(--ms-teal)",
  Draft: "#8c7280",
  "Sold Out": "var(--ms-rose)",
};
      ...
      <div className="mx-auto max-w-5xl space-y-6">
        ...
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <span className="flex h-12 w-12 items-center justify-center rounded-[18px] bg-[var(--ms-purple)] text-white shadow-[0_12px_30px_rgba(139,92,246,0.3)]">
          ...
          <div className="flex items-center gap-2">
            <span className="inline-flex items-center gap-1.5 rounded-full bg-white px-3 py-1.5 text-xs font-semibold text-[var(--ms-teal)] shadow-[0_4px_12px_rgba(13,27,42,0.08)]">
        ...
        <div className="grid gap-5 lg:grid-cols-[minmax(0,0.6fr)_minmax(0,0.4fr)]">
          ...
          <section className="rounded-[28px] border border-[var(--border-subtle)] bg-white p-5 shadow-[0_12px_40px_rgba(13,27,42,0.07)]">
            <div className="mb-4 flex items-center justify-between gap-4">
              ...
              <Link
                ...
                className="inline-flex items-center gap-1.5 rounded-full bg-[var(--ms-purple)] px-4 py-2 text-xs font-semibold text-white transition hover:brightness-110"
              >
              ...
              {MOCK_PRODUCTS.map((product) => (
                ...
                >
                  ...
                  <div className="flex shrink-0 items-center gap-2">
                    ...
                    <button className="text-[var(--ms-mauve)] transition hover:text-[var(--text-primary)]" type="button">
          ...
          <section className="rounded-[28px] border border-[var(--border-subtle)] bg-white p-5 shadow-[0_12px_40px_rgba(13,27,42,0.07)]">
            ...
            <p className="mt-4 rounded-[14px] bg-[var(--surface-card)] px-3 py-2 text-[10px] leading-4 text-[var(--ms-mauve)]">
        ...
        <section className="grid gap-3 sm:grid-cols-2">
          <div className="rounded-[24px] border border-[var(--border-subtle)] bg-white p-5">
            ...
            <p className="mt-2 text-lg font-semibold text-[var(--ms-purple)]">Shop Growth</p>
            <p className="mt-0.5 text-xs text-[var(--ms-mauve)]">Next billing: 30 May 2026 · KES 3,500</p>
            <div className="mt-3 flex gap-2">
              <button className="rounded-full border border-[var(--border-subtle)] px-3 py-1.5 text-xs font-semibold text-[var(--ms-mauve)] hover:border-[var(--ms-purple)] hover:text-[var(--ms-purple)]" type="button">
                Upgrade to Shop+
              </button>
              <button className="rounded-full border border-[var(--border-subtle)] px-3 py-1.5 text-xs text-[var(--ms-mauve)]" type="button">
                Manage plan
              </button>
            </div>
          </div>
          ...
          <div className="rounded-[24px] border border-[var(--border-subtle)] bg-white p-5">
            ...
            <div className="mt-3">
              <button
                className="inline-flex items-center gap-1.5 rounded-full bg-[var(--ms-purple)] px-3 py-1.5 text-xs font-semibold text-white transition hover:brightness-110"
                type="button"
              >
        ...
        <div className="flex flex-wrap gap-3">
          {[
            ...
            <Link
              key={action.label}
              href={action.href}
              className="inline-flex items-center gap-2 rounded-full border border-[var(--border-subtle)] bg-white px-4 py-2.5 text-sm font-semibold text-[var(--ms-mauve)] shadow-[0_4px_12px_rgba(13,27,42,0.05)] transition hover:border-[var(--ms-purple)] hover:text-[var(--ms-purple)]"
            >
              {action.icon}
              {action.label}
            </Link>
