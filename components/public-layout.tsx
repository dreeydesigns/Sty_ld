...
export function PublicLayout({
  ...
  return (
    <div className="flex min-h-dvh flex-col bg-[var(--surface-card)]">
      ...
      <header className="sticky top-0 z-40 border-b border-[var(--border-subtle)] bg-white/80 backdrop-blur-md">
        <div className="mx-auto flex max-w-lg items-center gap-3 px-4 py-3">
          {backHref && (
            <Link
              href={backHref}
              className="flex items-center gap-1 text-xs font-medium text-[var(--ms-mauve)] transition hover:text-[var(--color-primary)]"
            >
      ...
      {/* Minimal footer */}
      <footer className="border-t border-[var(--border-subtle)] bg-white py-5 text-center text-[11px] text-[var(--ms-mauve)]">
        <p>
